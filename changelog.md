...
### Correctness
- **Drillback auto-open, Forecast Quick Search, session-snapshot WO search restore, and FROM-link drillback restore now recover from EAM placeholder bind via direct-load.** When EAM's `selectRecord` chain throws on `f.tabXtype` deref (slow boots, screen-cache re-mounts), the form lands on an `<Auto-Generated>` placeholder model with `phantom===false` — the toast claims success while every detail field stays blank. v14.14.87's `recoverViaTabTickle` workaround set a non-HDR tab active to coax `MasterScreenController.loadTabData` into running, which worked but cost ~1.5–2s of visible tab-flicker; that workaround is removed in favour of calling `_tabView.loadRecordData()` directly on the mounted HDR form — the same entity-detail XHR EAM would have issued, no tab activation, no destruction/rebuild. Recovery measures ~3.5s on the verified bad reproduction with no visible flicker. Snapshot restore (`openMatchingGridRecord`, 7 call sites) was previously vulnerable to the same null-deref with no recovery wrapper at all; routing it through the new shared primitive (see Cleanup) wires it to the same recovery. Autofill (`navigateGridToRecord`) had no observed bug but inherits the protection for free now that it shares the helper.

### Cleanup
- **Auto-record-open consolidated behind one shared primitive.** `record-open.js` extracts `selectAndOpenGridRecord(grid, record, win, timeoutMs)` — fires `itemdblclick`, polls all frames for the record-view form, hands off to `verifyAndRecoverRecordBind` with the `itemdblclickThrew` flag. `openFirstGridRecord` (drillback auto-open + Forecast Quick Search) collapses to a thin caller that resolves `store.getAt(0)`; `openMatchingGridRecord` (session-snapshot restore) and `navigateGridToRecord` (autofill grid-to-record landing) drop their bespoke fire/poll loops and delegate to the same helper.
- **Drillback auto-open is always on; opt-in toggle removed.** The `drillbackAutoOpen` setting was added when the feature was unreliable on slow boots (battery throttling, screen-cache re-mounts); the new direct-load recovery closes those failure modes. Drops `apmGeneralSettings.drillbackAutoOpen` and the Settings → General toggle entirely; `boot.js` calls `handleDrillbackAutoOpen` unconditionally. Existing user setting entries become inert — keys are removed from the defaults shape, no migration needed.
- **Three obsolete protections removed.** `detectScreenCacheChurn()` (timing heuristic that skipped auto-open on slow APM boots or two session reloads within 3s) is replaced by precise content-based recovery — direct-load runs surgically when bind is incorrect, no need to predict it from boot duration. Phase 2 force-bind via `form.getForm().loadRecord(gridRecord)` is removed because it bound list-level fields only and produced a "looks correct but isn't" failure state (WO# visible, every detail field blank); Phase 3 `gridRecord.load()` detail re-fetch stays as the genuine last-resort. `getControllerReadiness` and `waitForGridControllerReady` (60 lines of investigative dead-end from the abandoned `tabXtype` readiness-gating attempt) are removed alongside their tests. Net `record-open.js` shrinks ~150 lines.

## v14.14.88 — Legacy column-order keys re-cleanup + runtime strip (2026-04-28)

### Correctness
- **Saved column orders no longer go silently invisible after upgrading from pre-v14.14.41 storage.** Some users carried bare-key `columnOrders` entries (`WSJOBS`, `OSOBJP`) past the v14.14.54 cleanup migration — likely round-tripped back from runtime memory after a stale cross-frame save, or imported from a backup that bypassed the import-time strip in `settings-io.js`. Composite-key lookups (`WSJOBS|<dataspyId>`) miss bare entries entirely, so every grid load served EAM defaults despite the user's saved layouts sitting right there in storage. Adds `legacy_column_orders_cleanup_v2` migration that runs across **7 boots** (new `runs: N` option in `MigrationManager`, tracked in `MIGRATIONS_RUN_COUNT_KEY`) so the storage cleanup survives a stale-tab writeback between boots. Pairs with `stripLegacyColumnOrderKeys()` in `autofill-prefs.js` running at every hydration path (`loadPresets`, `getPresets`, `getPresetsReadOnly`) so a bare key cannot survive in-session long enough to round-trip back to storage. The runtime strip stands down once v2 finalises — checks `MIGRATIONS_DONE_KEY` once and caches the answer — so there is no permanent overhead. One-time INFO log on first strip names the dropped keys for diagnostic visibility. Affected users will need to re-customize per (screen, dataspy) since the bare data cannot be auto-promoted to a composite key.

## v14.14.87 — Drillback / Quick-Jump Phantom-Bind Recovery (2026-04-28)

### Correctness
- **Auto-open drillback and Forecast Quick Jump now recover when EAM lands the form on its `<Auto-Generated>` placeholder.** On slow boots / screen-cache re-mounts (`itemdblclick` chain throws `"a is null"` inside `EAM.controller.common.grids.Grid.selectRecord` because the active tab's `tabXtype` content isn't built yet) and on direct-nav via `Nav.goTo` with `workordernum` in URL params (`waitAndOpenSingleResult` short-circuits because the view is already `'record'`), the form bound to a placeholder model with `phantom===false`, so prior recovery accepted the bad bind silently. `verifyAndRecoverRecordBind` now uses content-match (`isRecordBindCorrect` compares the bound record's WO/receipt/part field to the gridRecord's entity ID) and, on mismatch, runs `recoverViaTabTickle` — programmatic version of "click another tab and back" — which fires `MasterScreenController.loadTabData` via `tabchange` and lets EAM populate the form natively from the listview's active record. Initial cross-frame mistake (passing top `window` to look up an EAM controller that lives in a content iframe) hardened with `findEamControllerWindow` and DEBUG abort traces on every silent-return path. See `.planning/postmortems/postmortem-drillback-record-open-recovery-2026-04-28.md`.
...
### Quality
- **`frame-manager`, `ext-consistency`, and `message-router` gain smoke coverage.** 71 new tests across three files: frame-manager exercises `injectStylesIntoDoc` idempotency, `frame:attached`/`frame:gridMutation` emission via MutationObserver, navigation-guard short-circuit, discovery-burst URL pattern matching, debounce coalescing, `frame:ajaxBurst` timing, and stale-window observer cleanup; ext-consistency covers `init()` registering the three APMApi entry points, the global Ajax hooks (`auth-redirect` 401 handler and `login-detect` body-scanner), `bindAll`'s navigation-guard bail-out plus per-window `consistencyHook:*` iteration with error isolation and accessibility filtering, `triggerDiscoveryBurst` reentrancy guard and self-removing scheduler task, `triggerInjections` debouncing and autofill-running suppression, and `hookStoreReader` `MORERECORDPRESENT` extraction (XML and JSON metadata shapes); message-router validates origin gating, the full PTP routing table (heartbeat, click-away, start/cancelled/completed/stop, auth-diag, auth-gate, user-action), theme handshake reply (legacy and current shapes), hotkey whitelist + top-frame gate, colorcode filter de-dup, and PTP history evolution (COMPLETE preservation, legacy numeric migration, 30-day eviction). Adds two test-only reset exports (`_resetFrameManagerForTest`, `_resetExtConsistencyForTest`) so module-level state (`_gridObservers`, `_discoveryBurstRegistered`, `_burstActive`, `injectionTO`) does not leak between tests. Implements P3.4 from the 2026-04-26 audit — closes the largest test-coverage gap in `core/` and unlocks future restructuring of the three orchestrators.
